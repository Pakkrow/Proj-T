...
const PokeCard = ({ name, lvl = 0 }) => {
    ...
    async function getPokeURL(params) {
        try {
            ...
            console.log("params.lvl == " + params.lvl);
        ...
        const lvldStats = stats.map(stat => {
            if (stat && stat.stat && stat.stat.name) {
                if (stat.stat.name === "hp") {
                    stat.base_stat = Math.round((((2 * stat.base_stat + 31) * lvl) / 100) + lvl + 10);
                } else {
                    stat.base_stat = Math.round((((2 * stat.base_stat + 31) * lvl) / 100) + 5);
                }
                return stat;
            } else {
    ...
    const getTypeUrls = () => {
        const newTypePaths = pokeTypes.map(pokeType => {
    ...
    const displaySecondType = () => {
        if (pokeTypes.length === 2) 
            ...
            return <div style={{ height: "35.5px", marginTop: "5px", marginBottom: "5px" }}></div>;
    };
    ...
    return (
        <div className='pokeCard'>
            <p style={{ textTransform: "capitalize" }}>{pokeName ? pokeName : "Not yet"}</p>
            <img className="mons" src={isShiny ? shinyFrontSprite : defFrontSprite} alt="Pokemon" />
            <img className="types" src={typePath[0]} alt="Type 1" />
            {displaySecondType()}
            <button onClick={() => setIsShiny(!isShiny)} style={{ all: "unset" }}>
                <img className={isShiny ? "shinyLogoON" : "shinyLogoOFF"} src={shinyLogo} alt="Shiny Toggle" />
            </button>
            {displayLvl()}
            <ul className='bsList'>
